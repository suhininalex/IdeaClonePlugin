...
public class Edge<T, S extends Iterable<T>> implements Iterable<T> {
	...
	private void split(Suffix<T,S> suffix, ActivePoint<T,S> activePoint) {
        /* crutch on. Need for correct computing node length */
        int buf = end;
    ...
    public String toDebugString() {
        ...
        if  (this.isTerminating() && this.getTerminal().getSuffixLink()!=null)
            return sb.toString()+"{"+this.getTerminal().getSuffixLink().incomingEdge+"}";
        else return sb.toString()+"{none}";
    }
