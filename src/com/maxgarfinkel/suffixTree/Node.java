...
public class Node<T,S extends Iterable<T>> implements Iterable<Edge<T,S>> {
    ...
    /** Added!
     * CloneClass - for clones info
     * length - length from root to current node
     */
    ...
    void addCloneRecursive(int startPosition){
        Node<T,S> currentNode=this;
        while(currentNode.cloneClass.addClone(startPosition)){
            if(currentNode.incomingEdge!=null){
	...
	Node(Edge<T,S> incomingEdge, Sequence<T,S> sequence, SuffixTree<T,S> tree) {
        ...
        if (incomingEdge!=null) {
            length=incomingEdge.parentNode.getLength()+incomingEdge.getLength();
            cloneClass = new CloneClass(sequence.masterSequence, getLength());
            cloneClass.addClone(incomingEdge.getEnd() - getLength());
        } else {
            length = 0;
            cloneClass = new CloneClass(sequence.masterSequence, getLength());
        }
	...
	void insert(Suffix<T,S> suffix, ActivePoint<T,S> activePoint) {
		...
		if (edges.containsKey(item)) {
            ...
            this.addCloneRecursive(suffix.getStartPosition());
        } else {
